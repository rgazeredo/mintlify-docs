---
title: "Bloquear Edição de Informações"
api: "POST /groups/update-locked"
description: "Controlar quem pode editar informações do grupo"
---

Define se apenas administradores podem modificar as informações do grupo (nome, descrição, imagem e outras configurações).

## Controle de Edição

<AccordionGroup>
  <Accordion title="Modo Bloqueado" icon="lock">
    Quando apenas administradores podem editar informações.

    **Configuração `locked: true`:**
    - Apenas administradores editam informações do grupo
    - Membros comuns não podem alterar nome
    - Descrição protegida contra alterações
    - Imagem do grupo só alterada por admins
    - Outras configurações restritas
    - Mantém consistência e identidade do grupo
  ...
  <Accordion title="Modo Desbloqueado" icon="lock-open">
    Quando qualquer participante pode editar informações.

    **Configuração `locked: false`:**
    - Todos os membros podem editar informações
    - Permite personalização colaborativa
    - Maior liberdade para participantes
    - Útil para grupos informais
    - Pode resultar em mudanças frequentes
    - Requer confiança entre membros

    Adequado para grupos familiares e informais.

  </Accordion>

  <Accordion title="Escopo de Restrição" icon="sliders">
    O que é afetado pelo bloqueio de edição.

    **Afeta:**
    - Nome do grupo
    - Descrição do grupo
    - Imagem de perfil do grupo
    - Outras configurações de informação

    **Não afeta:**
    - Adição ou remoção de participantes
    - Envio de mensagens
    - Outras permissões de participação
    - Configurações de anúncio (announce)

    Cada permissão é controlada separadamente.

  </Accordion>

  <Accordion title="Casos de Uso" icon="lightbulb">
    Situações para usar cada modo de bloqueio.
    ...
    **Modo Bloqueado (`true`):**
    ...
    - Organizações com políticas rígidas
    - Grupos de notícias e anúncios
...
<ParamField path="locked" type="boolean" required>
  Controla quem pode editar informações. <br />
...
<Info>
  **Nota:** Esta configuração é também conhecida como "restrict" em algumas
  implementações do WhatsApp. Ela protege as informações básicas do grupo contra
  edições não autorizadas.
</Info>

<RequestExample>

```bash cURL - Bloquear edição
curl -X POST https://zapclass.com.br/api/groups/update-locked \
...
```javascript JavaScript - Bloquear edição
const response = await fetch("https://zapclass.com.br/api/groups/update-locked", {
  method: "POST",
  headers: {
...
if (data.success) {
  console.log("Edição de informações bloqueada - apenas admins podem editar");
}
```
...
async function toggleGroupLock(groupJid) {
  ...
  const updateResponse = await fetch(
    "https://zapclass.com.br/api/groups/update-locked",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
```javascript JavaScript - Configurar com notificação
async function setGroupLockWithNotification(
  ...
  if (notifyMembers) {
    // Envia mensagem no grupo informando a mudança
    const message = locked
      ? "🔒 *Configuração Atualizada*\n\nApenas administradores podem editar as informações do grupo (nome, descrição, imagem) a partir de agora."
      : "🔓 *Configuração Atualizada*\n\nTodos os participantes podem editar livremente as informações do grupo.";
...
// Uso - bloquear edição e notificar
await setGroupLockWithNotification("120363308883996631@g.us", true, true);
```

```javascript JavaScript - Aplicar política em múltiplos grupos
...
// Uso - bloquear edição em todos os grupos corporativos
...
console.log("=== RESULTADOS DA POLÍTICA ===");
...
```javascript JavaScript - Configuração completa de grupo
async function setupCorporateGroup(groupJid) {
  console.log("Configurando grupo corporativo...\n");

  try {
    // 1. Bloquear edição de informações
    await fetch("https://zapclass.com.br/api/groups/update-locked", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP",
    ...
    console.log("✓ Edição bloqueada (apenas admins)");

    // 2. Ativar modo anúncio
    await fetch("https://zapclass.com.br/api/groups/updateAnnounce", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP",
    ...
    console.log("✓ Modo anúncio ativado");

    // 3. Atualizar descrição
    const description =
      "📋 GRUPO CORPORATIVO\n\n" +
      "Este é um grupo oficial da empresa.\n" +
      "Apenas administradores podem editar informações e enviar mensagens.\n\n" +
      "Para dúvidas, entre em contato com os administradores.";
    ...
    console.log("\n✓ Grupo configurado como corporativo com sucesso!");
  } catch (error) {
...
```php PHP - Bloquear edição
<?php
$ch = curl_init('https://zapclass.com.br/api/groups/update-locked');
...
if ($data['success']) {
    echo "Edição bloqueada - apenas admins podem editar informações\n";
} else {
...
```php PHP - Aplicar política em múltiplos grupos
<?php
...
class GroupLockManager {
    ...
    public function setLocked($groupJid, $locked) {
        $data = $this->makeRequest('/groups/update-locked', [
            'groupjid' => $groupJid,
            'locked' => $locked
        ]);
    ...
    public function getCurrentState($groupJid) {
        ...
        return [
            ...
            'description' => $locked
                ...
                : 'Todos os participantes podem editar informações'
        ];
    }
    ...
    public function setupCorporateGroup($groupJid) {
        ...
        // Ativa modo anúncio
        $this->makeRequest('/groups/updateAnnounce', [
            'groupjid' => $groupJid,
            'announce' => true
        ]);
        ...
        $this->makeRequest('/groups/updateDescription', [
            'groupjid' => $groupJid,
...
try {
    ...
    echo "✓ Edição bloqueada\n\n";

    // Alternar bloqueio
    $manager->toggle("120363308883996631@g.us");
    echo "✓ Bloqueio alternado\n\n";

    // Aplicar política a múltiplos grupos
    $groups = [
        "120363308883996631@g.us",
        "120363308883996632@g.us"
    ];
    ...
    foreach ($results as $groupJid => $result) {
        if ($result['success']) {
            echo "✓ {$groupJid} - Bloqueado\n";
        } else {
            echo "✗ {$groupJid} - {$result['error']}\n";
        }
    }
    ...
    echo "✓ Grupo configurado como corporativo\n";
} catch (Exception $e) {
    echo "Erro: " . $e->getMessage();
}
?>
...
{
  "success": false,
  "message": "O parâmetro 'locked' deve ser um booleano (true ou false)"
}
```
...
{
  "success": false,
  "message": "Você precisa ser administrador do grupo para alterar permissões de edição"
}
```
...
{
  "success": false,
  "message": "Erro ao atualizar permissões do grupo. Tente novamente"
}
```
