...
<Info>
  **Nota:** Esta configuração é também conhecida como "restrict" em algumas implementações do WhatsApp. Ela protege as informações básicas do grupo contra edições não autorizadas.
</Info>
...
async function toggleGroupLock(groupJid) {
  ...
  console.log(`Estado atual: ${currentLocked ? 'Bloqueado (apenas admins)' : 'Desbloqueado (todos)'}`);
  console.log(`Alterando para: ${newLocked ? 'Bloqueado (apenas admins)' : 'Desbloqueado (todos)'}`);

  // Alterna o bloqueio
  const updateResponse = await fetch("https://zapclass.com.br/api/group/updateLocked", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Token: "zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk",
...
async function setGroupLockWithNotification(groupJid, locked, notifyMembers = false) {
  const response = await fetch("https://zapclass.com.br/api/group/updateLocked", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function setupCorporateGroup(groupJid) {
  ...
  try {
    ...
    const description = "📋 GRUPO CORPORATIVO\n\n" +
