...
api: "POST /groups/update-participants"
...
<AccordionGroup>
  ...
  <Accordion title="Aprovar Solicitações" icon="check">
    ...
    **Ação: `approve`**
    ...
    - Lista de pendentes visível em `/groups/info`
...
curl -X POST https://zapclass.com.br/api/groups/update-participants \
...
const response = await fetch(
  "https://zapclass.com.br/api/groups/update-participants",
  {
    method: "POST",
    headers: {
...
class GroupParticipantManager {
  ...
  async executeAction(groupJid, action, participants) {
    const response = await fetch(`${this.baseUrl}/groups/update-participants`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Token: this.token,
...
async function syncGroupParticipants(groupJid, desiredParticipants) {
  ...
  // Adiciona novos
  if (toAdd.length > 0) {
    await fetch("https://zapclass.com.br/api/groups/update-participants", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP",
...
<?php
$ch = curl_init('https://zapclass.com.br/api/groups/update-participants');
...
class GroupParticipantManager {
    ...
    private function executeAction($groupJid, $action, $participants) {
        $ch = curl_init($this->baseUrl . '/groups/update-participants');
    ...
    public function getParticipants($groupJid) {
        $ch = curl_init($this->baseUrl . '/groups/info');
