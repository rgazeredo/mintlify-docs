...
<Info>
  **Formato de números:** Use apenas dígitos no formato internacional (código do país + DDD + número). Exemplo: `5511999999999` para Brasil.
</Info>
...
const response = await fetch("https://zapclass.com.br/api/group/updateParticipants", {
  method: "POST",
  headers: {
...
class GroupParticipantManager {
  ...
  async add(groupJid, numbers) {
    ...
    console.log(`✓ ${result.results?.success?.length || 0} participantes adicionados`);
...
// Uso
const manager = new GroupParticipantManager("zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk");
...
async function approvePendingRequests(groupJid) {
  ...
  const numbers = pending.map(p => p.number || p.jid.replace('@s.whatsapp.net', ''));
...
async function syncGroupParticipants(groupJid, desiredParticipants) {
  ...
  const current = groupInfo.group.participants.map(p =>
    p.number || p.jid.replace('@s.whatsapp.net', '')
  );
