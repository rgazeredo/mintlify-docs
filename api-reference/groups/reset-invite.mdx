...
<Warning>
  **Atenção de Segurança:** Ao resetar o código, todos os links compartilhados anteriormente se tornam inválidos. Certifique-se de comunicar o novo link apenas para pessoas autorizadas.
</Warning>
...
const response = await fetch("https://zapclass.com.br/api/group/resetInviteCode", {
  method: "POST",
  headers: {
...
async function resetInviteCodeWithConfirmation(groupJid) {
  ...
  // Reseta o código
  const resetResponse = await fetch("https://zapclass.com.br/api/group/resetInviteCode", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Token: "zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk",
  ...
  const resetLink = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.error(`[${new Date().toLocaleString('pt-BR')}] ✗ Erro: ${data.message}`);
      }
    } catch (error) {
      console.error(`[${new Date().toLocaleString('pt-BR')}] ✗ Erro:`, error.message);
    }
  };
...
async function resetAndNotify(groupJid, adminNumbers) {
  ...
  const message = `🔐 *Link de Convite Atualizado*\n\n` +
