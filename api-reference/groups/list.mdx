...
<Tip>
  **Dica de Performance:** Use `noparticipants=true` quando precisar apenas de informações básicas dos grupos. Isso reduz significativamente o tamanho da resposta e o tempo de processamento.
</Tip>
...
const response = await fetch(
  "https://zapclass.com.br/api/group/list",
  {
    method: "GET",
    headers: {
...
async function listGroups(options = {}) {
  ...
  const url = `https://zapclass.com.br/api/group/list${params.toString() ? '?' + params.toString() : ''}`;
...
async function analyzeGroups() {
  ...
  groups.forEach(group => {
    ...
    if (!stats.largestGroup || participantCount > (stats.largestGroup.participants?.length || 0)) {
      stats.largestGroup = group;
    }

    if (!stats.smallestGroup || participantCount < (stats.smallestGroup.participants?.length || 0)) {
  ...
  console.log(`\nMaior grupo: ${stats.largestGroup.name} (${stats.largestGroup.participants?.length || 0} membros)`);
  console.log(`Menor grupo: ${stats.smallestGroup.name} (${stats.smallestGroup.participants?.length || 0} membros)`);
...
async function filterGroups(criteria) {
  ...
  if (criteria.minParticipants) {
    filtered = filtered.filter(g =>
      (g.participants?.length || 0) >= criteria.minParticipants
    );
  }
  ...
  if (criteria.maxParticipants) {
    filtered = filtered.filter(g =>
      (g.participants?.length || 0) <= criteria.maxParticipants
    );
  }
