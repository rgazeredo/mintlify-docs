...
<Warning>
  **Restrições:** A imagem deve ser JPEG com resolução máxima de 640x640 pixels.
  Outros formatos ou tamanhos maiores serão rejeitados pelo WhatsApp.
</Warning>

<RequestExample>

```bash cURL - Atualizar com URL
curl -X POST https://zapclass.com.br/api/groups/update-image \
...
const response = await fetch("https://zapclass.com.br/api/groups/update-image", {
  method: "POST",
  headers: {
  ...
  return new Promise((resolve, reject) => {
    reader.onload = async (e) => {
      ...
      const response = await fetch(
        "https://zapclass.com.br/api/groups/update-image",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
async function setSeasonalImage(groupJid, event, durationDays) {
  ...
  await fetch("https://zapclass.com.br/api/groups/update-image", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP",
...
<?php
$ch = curl_init('https://zapclass.com.br/api/groups/update-image');
...
class GroupImageManager {
    ...
    public function updateFromUrl($groupJid, $imageUrl) {
        $data = $this->makeRequest('/groups/update-image', [
            'groupjid' => $groupJid,
            'image' => $imageUrl
        ]);
    ...
    public function updateFromFile($groupJid, $imagePath) {
        ...
        return $this->makeRequest('/groups/update-image', [
            'groupjid' => $groupJid,
            'image' => $base64
        ]);
    }
