...
curl -X POST https://zapclass.com.br/api/groups/leave \
...
GROUP_INFO=$(curl -s -X POST https://zapclass.com.br/api/groups/info \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP" \
...
const response = await fetch("https://zapclass.com.br/api/groups/leave", {
  method: "POST",
  headers: {
...
async function leaveGroupSafely(groupJid) {
  ...
  // Sai do grupo
  const leaveResponse = await fetch("https://zapclass.com.br/api/groups/leave", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer zc_6FrX2qjM4bHT9pR6cC0sD8wLkG3nE2tZfV5uN1aP",
...
<?php
$ch = curl_init('https://zapclass.com.br/api/groups/leave');
...
class GroupManager {
    ...
    public function leaveGroup($groupJid, $checkAdmin = true) {
        ...
        return $this->makeRequest('/groups/leave', [
            'groupjid' => $groupJid
        ]);
    }
