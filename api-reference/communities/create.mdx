...
<Info>
  **Próximos passos:** Após criar a comunidade, use o endpoint
...
async function createCommunity(name) {
  ...
  console.log(
    `  Grupo de anúncios: ${data.community.announcementGroup || "Criado"}`
  );
...
async function createAndSetupCommunity(name, description) {
  ...
  const createResponse = await fetch(
    "https://zapclass.com.br/api/community/create",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    for (const groupConfig of initialGroups) {
      try {
        const groupResponse = await fetch(
          "https://zapclass.com.br/api/group/create",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
