...
Cria uma nova comunidade do WhatsApp, que permite agrupar múltiplos grupos relacionados sob uma única administração centralizada.

## Estrutura de Comunidades

<AccordionGroup>
  <Accordion title="O que são Comunidades" icon="users-rectangle">
    Entenda o conceito de Comunidades no WhatsApp.

    **Definição:**
    - Estrutura hierárquica para organizar grupos
    - Permite administração centralizada de múltiplos grupos
    - Cada comunidade tem um grupo de anúncios principal
    - Grupos vinculados compartilham contexto comum
    - Facilita gestão em larga escala

    **Benefícios:**
    - Organização de grupos relacionados
    - Comunicados centralizados via grupo de anúncios
    - Gerenciamento simplificado de múltiplos grupos
    - Identidade visual unificada
    - Controle administrativo centralizado
  </Accordion>

  <Accordion title="Criação e Configuração" icon="plus">
    Como funciona a criação de uma comunidade.

    **Processo:**
    - Comunidade criada com nome escolhido
    - Criador torna-se administrador automaticamente
    - Grupo de anúncios criado automaticamente
    ...
    **Grupo de Anúncios:**
    ...
    - Todos os membros dos grupos vinculados têm acesso
    - Ideal para avisos e informações importantes
  </Accordion>

  <Accordion title="Limitações e Requisitos" icon="circle-info">
    Restrições aplicadas às comunidades.

    **Limites do WhatsApp:**
    - Número máximo de comunidades por conta
    - Limite de grupos por comunidade
    - Nome da comunidade tem limite de caracteres
    - Apenas o criador tem certos privilégios

    **Requisitos:**
    - Conexão válida e ativa
    - Permissões para criar comunidades
    - Nome não pode estar vazio
    - Conta em bom estado no WhatsApp
  </Accordion>

  <Accordion title="Casos de Uso" icon="lightbulb">
    Situações ideais para usar comunidades.

    **Aplicações:**
    - Organizações com múltiplos departamentos
    - Escolas com turmas diferentes
    - Condomínios com blocos separados
    - Empresas com filiais regionais
    - Eventos com múltiplas áreas
...
<Info>
  **Próximos passos:** Após criar a comunidade, use o endpoint `/community/updategroups` para vincular grupos existentes ou criar novos grupos dentro da comunidade.
</Info>

<RequestExample>

```bash cURL - Criar comunidade
curl -X POST https://zapclass.com.br/api/community/create \
...
RESPONSE=$(curl -s -X POST https://zapclass.com.br/api/community/create \
  -H "Content-Type: application/json" \
  -H "Token: zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk" \
  -d '{
...
const response = await fetch("https://zapclass.com.br/api/community/create", {
  method: "POST",
  headers: {
...
if (data.success) {
  ...
  console.log("Grupo de anúncios:", data.community.announcementGroup);
}
```

```javascript JavaScript - Criar com validação
async function createCommunity(name) {
  ...
  console.log(`  Grupo de anúncios: ${data.community.announcementGroup || 'Criado'}`);
...
// Uso
try {
  const community = await createCommunity("Condomínio Parque das Flores");
...
async function createAndSetupCommunity(name, description) {
  // Cria a comunidade
  const createResponse = await fetch("https://zapclass.com.br/api/community/create", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Token: "zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk",
  ...
  // Configura descrição do grupo de anúncios (se tiver o JID)
  if (community.community.announcementGroup) {
    try {
      ...
      console.log("✓ Descrição do grupo de anúncios configurada");
    } catch (error) {
      console.warn("Falha ao configurar descrição:", error.message);
    }
  }
...
const community = await createAndSetupCommunity(
  "Escola Municipal",
  "📚 Comunidade da Escola Municipal\n\nCanal oficial de comunicação com pais e alunos."
);
```

```javascript JavaScript - Criar múltiplas comunidades
async function createMultipleCommunities(names) {
  ...
  for (const name of names) {
    try {
      ...
      // Aguarda 2 segundos entre criações
...
async function createCommunityWithStructure(config) {
  ...
  if (initialGroups && initialGroups.length > 0) {
    ...
    for (const groupConfig of initialGroups) {
      try {
        const groupResponse = await fetch("https://zapclass.com.br/api/group/create", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Token: "zt_BppKZWtzvj1X2SbJAuOSkA0Z63uDBQ0wkb28NJaloKeOUnHk",
        ...
        if (groupData.success) {
          ...
          console.log(`  ✓ Grupo criado: ${groupConfig.name}`);
        }

        await new Promise(resolve => setTimeout(resolve, 1000));
      } catch (error) {
        console.warn(`  ✗ Falha ao criar grupo: ${groupConfig.name}`);
      }
    }
  }

  console.log("\n✓ Estrutura da comunidade criada com sucesso!");
...
<?php
$ch = curl_init('https://zapclass.com.br/api/community/create');
...
// Uso
try {
    $community = createCommunity("Condomínio Parque das Flores");
    print_r($community);
} catch (Exception $e) {
...
```php PHP - Criar múltiplas comunidades
<?php
...
class CommunityManager {
    ...
    public function createWithGroups($name, $groupNames) {
        ...
        foreach ($groupNames as $groupName) {
            try {
                $groupData = $this->makeRequest('/group/create', [
                    'name' => $groupName,
                    'participants' => []
                ]);
                ...
                if ($groupData['success']) {
                    ...
                    echo "  ✓ Grupo criado: {$groupName}\n";
                }

                sleep(1);
            } catch (Exception $e) {
                echo "  ✗ Falha ao criar grupo: {$groupName}\n";
            }
        }
...
{
  "success": false,
  "message": "O nome da comunidade não pode estar vazio"
}
```
...
{
  "success": false,
  "message": "Sem permissão para criar comunidades"
}
```
...
{
  "success": false,
  "message": "Limite de criação de comunidades atingido. Tente novamente mais tarde"
}
```
